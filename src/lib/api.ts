...
// All requests route through Azure Functions proxy (/api/umls, /api/rxnav)
...
export async function searchUMLS(params: SearchUMLSRequest): Promise<SearchUMLSResponse> {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    pagePromises.push(
      fetch(`/api/umls/search/2025AB?${queryParams.toString()}`)
        .then(response => {
          if (!response.ok) {
    ...
    const batchPromises = batch.map(async (result) => {
      try {
        const response = await fetch(
          `/api/umls/content/2025AB/CUI/${result.ui}`
        );
        ...
        return {
          ...
          preferredAtom: data.result?.preferredAtom || result.name,
        };
      } catch (error) {
    ...
    batchResults.forEach((details, batchIndex) => {
      if (details) {
        ...
        enrichedResults[resultIndex] = {
          ...
          preferredAtom: details.preferredAtom,
        };
      }
    });
  }
...
export async function getConceptDetails(cui: string, vocabularies?: string[]): Promise<any> {
  ...
  try {
    ...
    const [conceptResponse, atomsResponse] = await Promise.all([
      fetch(`/api/umls/content/2025AB/CUI/${cui}?includeSuppressible=true&includeObsolete=true`),
      fetch(`/api/umls/content/2025AB/CUI/${cui}/atoms?sabs=${vocabFilter}&pageSize=100&includeSuppressible=true&includeObsolete=true`)
    ]);
...
export async function getAtomDetails(aui: string): Promise<any> {
  try {
    const response = await fetch(
      `/api/umls/content/2025AB/AUI/${aui}?includeSuppressible=true&includeObsolete=true`
    );
...
export async function getAncestors(vocabulary: string, code: string): Promise<any[]> {
  try {
    const response = await fetch(
      `/api/umls/content/2025AB/source/${vocabulary}/${code}/ancestors?includeSuppressible=true&includeObsolete=true`
    );
...
export async function getDescendants(vocabulary: string, code: string): Promise<any[]> {
  ...
  try {
    ...
    while (hasMorePages) {
      const response = await fetch(
        `/api/umls/content/2025AB/source/${vocabulary}/${code}/descendants?pageSize=${pageSize}&pageNumber=${pageNumber}&includeSuppressible=true&includeObsolete=true`
      );
...
async function getRxNormDescendants(rxcui: string): Promise<any[]> {
  try {
    ...
    const response = await fetch(
      `/api/rxnav/rxcui/${rxcui}/related.json?tty=IN+SCD+SBD+SCDC+SBDC`
    );
...
export async function getRxNormAncestors(rxcui: string): Promise<any[]> {
  try {
    ...
    const url = `/api/umls/content/2025AB/source/RXNORM/${rxcui}/relations`;
...
async function getRxNormAttributesNDC(rxcui: string, cui?: string): Promise<any[]> {
  ...
  try {
    ...
    const rxnormResponse = await fetch(
      `/api/umls/content/2025AB/source/RXNORM/${rxcui}/attributes?pageSize=1000&includeSuppressible=true&includeObsolete=true`
    );
    ...
    if (cui) {
      ...
      const cuiResponse = await fetch(
        `/api/umls/content/2025AB/CUI/${cui}/attributes?pageSize=1000&includeSuppressible=true&includeObsolete=true`
      );
...
export async function getRxNormToNDC(rxcui: string, cui?: string): Promise<any[]> {
  try {
    ...
    try {
      const response = await fetch(
        `/api/rxnav/rxcui/${rxcui}/ndcs.json`
      );
...
export async function getCUIAncestors(cui: string): Promise<any[]> {
  try {
    ...
    const response = await fetch(
      `/api/umls/content/2025AB/CUI/${cui}/relations?pageSize=1000`
    );
    ...
    for (const rel of hierarchicalParents) {
      ...
      try {
        // The proxy rewrites URLs in the response, so atomClusterUrl is already
        // a proxy-relative path like /api/umls/content/2025AB/source/...
        console.log(`[CUI ANCESTORS] Fetching atoms from: ${atomClusterUrl}/atoms`);

        // Fetch the atoms list from the SourceAtomCluster
        const atomsResponse = await fetch(`${atomClusterUrl}/atoms`);
...
export async function getConceptRelations(cui: string): Promise<any[]> {
  try {
    const response = await fetch(
      `/api/umls/content/2025AB/CUI/${cui}/relations?includeSuppressible=true&includeObsolete=true`
    );
